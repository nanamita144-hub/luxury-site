...
export default function ContactClient({ heading, intro, details }: {
  heading: string;
  intro: string;
      ...
      <StagePage>
        ...
        <div className="mt-14 grid grid-cols-1 gap-12 sm:grid-cols-2 sm:gap-16">
            ...
            <div className="border-t border-[#b89b5e]/25">
              {details.map((r) => (
                <div key={r.k} className="border-b border-[#b89b5e]/25 py-5">
                  <p className="font-sans text-[10px] uppercase tracking-[0.45em] text-[#a89678]">{r.k}</p>
                  <p className="mt-2 whitespace-pre-line font-serif text-lg font-light uppercase tracking-[0.1em] text-[#d8c28a] sm:text-xl">{r.v}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="flex flex-col gap-7 pt-2">
              ...
              {sent ? (
                ...
                <button type="submit" className="mt-2 self-start border border-[#b89b5e] bg-transparent px-8 py-4 font-sans text-[11px] uppercase tracking-[0.45em] text-[#d8c28a] transition-colors hover:bg-[#b89b5e] hover:text-black active:opacity-70">
                  Send
                </button>
              )}
            </form>
...
function Field({ id, label, type, placeholder, required }: { id: string; label: string; type: string; placeholder: string; required?: boolean }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="font-sans text-[10px] uppercase tracking-[0.45em] text-[#a89678]">{label}</label>
